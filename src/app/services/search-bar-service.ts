...
export class SearchBarService {
    ...
    private geneListSource = new BehaviorSubject<any[]>([]);
    ...
    constructor(private geneService: ElasticGeneSearch,
        ...
        this.query = '';
        this.options = [
            new SearchOption('Cohort', 'dataset', ['Demo', 'Mitochondria', 'Acute Care Trios', 'Acute Care Probands', 'Epileptic Encephalopathies', 'Brain Malformations', 'Leukodystrophies', 'ICCon', 'KidGen', 'Genetic Immunology', 'Childranz', 'HIDDEN', 'Cardiac', 'Neuromuscular'], 'Demo'),
        ];
    }
    ...
    searchWithMultipleParams(params: Params): Promise<VariantAutocompleteResult<any>[]> {
        ...
        if(panel.length && panelGroup === 'agha'){
            ...
            return this.genomicsEnglandService.getPanel(panel).toPromise().then((data) => {
                ...
                regions = data.genesData.genes.filter(gene => {
                    let found = false;
                    ...
                    return found;
                  }).filter(e => e.gene_data.ensembl_genes.GRch37).map(e =>  e.gene_data.ensembl_genes.GRch37['82'].location);
                ...
                if(regions.length === 0){
                    ...
                    if(queries.length === 0){
                        this.autocompleteError = QUERY_LIST_ERROR;
                    }else{
                        return <any>Promise.all(queries).then(v => {
                            let bestMatches = v.map(q => q[0]);
                            return bestMatches;
