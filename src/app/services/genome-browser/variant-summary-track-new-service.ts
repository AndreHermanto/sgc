...
import { VariantSummarySearchNewService } from '../variant-summary-search-new-service';
...
class VariantPin {
    constructor(public pos: number,
                public val: number,
                public name: string,
                public variant: VariantSummaryNew) {
    }
}

@Injectable()
export class VariantSummaryTrackNewService implements TrackService {
    track: any;
    trackLabel: any;
    highlightedVariant = new Subject<VariantSummaryNew>();
    clickedVariant = new Subject<VariantSummaryNew>();
    data: any;
    ...
    constructor(private searchService: VariantSummarySearchNewService,
        ...
        this.highlightedVariant.subscribe((v: VariantSummaryNew) => {
    ...
    private createDataMethod(): () => any {
        const createPin = (variant: VariantSummaryNew) => {
        ...
        return tnt.track.data.async()
            .retriever((loc: any) => {
                ...
                if (this.searchService.startingRegion.start !== loc.from ||
                    ...
                    return regionAutocomplete.searchSummaryNew(this.searchService, this.searchService.lastQuery.options).then(() => {
    ...
    private highlightPin(v: VariantSummaryNew) {
    ...
    private unHighlightPin(v: VariantSummaryNew) {
        const hash = this.variantHash(v);
        const e = <HTMLElement>d3.select(`[data-variant-id='${ hash }']`)[0][0];
        const dPin = d3.select(e);
        const circle = dPin.select('circle');
    ...
    private variantName(variant: VariantSummaryNew) {
        return variant.chr +
            variant.start +
            variant.ref +
            variant.alt +
