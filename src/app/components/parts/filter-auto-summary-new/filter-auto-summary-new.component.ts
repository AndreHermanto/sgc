...
import { FilterSummaryNewService } from '../../../services/filter-summary-new.service';

type FilterStream = [string, boolean];

@Component({
  selector: 'app-filter-auto-summary-new',
  templateUrl: './filter-auto-summary-new.component.html',
  styleUrls: ['./filter-auto-summary-new.component.css'],
  providers: [FilterSummaryNewService]
})
export class FilterAutoSummaryNewComponent implements OnInit, OnDestroy, AfterViewInit {
  @Input() variants: VariantSummaryNew[];
  @Output() output: EventEmitter<VariantSummaryNew[]> = new EventEmitter();
  show = false;
  selectedIndex = 0;
  currentFilter = '';
  autoComplete: string[] = [];
  ...
  private unfiltered: VariantSummaryNew[];

  constructor(private ts: TableSummaryNewService, private fs: FilterSummaryNewService) {
  ...
  ngAfterViewInit() {
      ...
      this.subscriptions.push(this.filter.debounceTime(120).subscribe(filterStream => {
          ...
          if (this.fs.isCommand(this.currentFilter)) {
              ...
              let variants: VariantSummaryNew[] = [];
              if (!filter) {
                  variants = this.unfiltered;
              } else {
                  variants = this.unfiltered.filter((v: VariantSummaryNew) => {
                      let found = false;
  ...
  reset(variants: VariantSummaryNew[]) {
