...
export class VariantsTablePaginatedComponent implements OnInit, OnDestroy {
    ...
    searchCLinical(variant){
        if(this.cohortAccessClinical.includes(this.selectedCohort)){
            ...
            const obj = {query: `${query[0]}:${query[1]}`,build:"GRCh37", cohort: this.selectedCohort, panel:"",ref:query[2], alt:query[3], het: 'true', hom: 'true', conj:'false', conjSamples: 'false', timestamp: Date.now()};
